...
in
{
  ...
  home.activation.windowsSync = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    _windowsSync() {
      ...
      cp ${dotfiles}/config/intellij/ideavimrc "/mnt/c/Users/$user/.ideavimrc" || return 1
      mkdir -p "/mnt/c/Users/$user/.glzr/glazewm" || return 1
      cp ${dotfiles}/config/glazewm/config.yaml "/mnt/c/Users/$user/.glzr/glazewm/config.yaml" || return 1
  ...
  home.activation.wingetSync = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    _wingetSync() {
      ...
      [ -e "$winget" ] || return 0
      "$winget" import -i "$(wslpath -w ${dotfiles}/config/winget/packages.json)" \
